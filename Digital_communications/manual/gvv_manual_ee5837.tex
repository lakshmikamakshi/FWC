...
\provdecommand{\dec}[2]{\ensuremath{\overset{#1}{\underset{#2}{\gtrless}}}}
...
\fi
\thesection{Uniform Random Numbers}
...
\\
\solution Download the following files .Uncomment the required lines and execute the  C program.
\begin{lstlisting}
codes/exrand.c
codes/coeffs.h
\end{lstlisting}
...
\end{equation}\\
\solution Mean $\mu_X$ and variance $\sigma_X^2$ of a Random Variable $X$ are given by
\begin{align}
	\label{eq:mean_exp}
...
\solution The samples for $U$ are loaded from uni.dat file generated in problem \ref{prob:print_uni}. The CDF of $V$ is plotted in the following figure using the code below, 
\begin{lstlisting}
...
\solution Loading the samples from uni1.dat and uni2.dat in python, the CDF is plotted below
\begin{figure}[H]
\centering
\includegraphics[width=\columnwidth]{./figs/gau_cdf.pdf}
...
\solution The PDF of $T$ is plotted using the code below
\begin{lstlisting}
...
\solution The theoretical and numerical plots for the CDF and PDF of $T$ closely match in plot.
\end{enumerate}
\section{BPSK}
\begin{enumerate}
\item
