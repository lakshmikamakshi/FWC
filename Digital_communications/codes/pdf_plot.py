...
#randvar = np.loadtxt('nat.dat',dtype='double')
...
def uni_pdf(x):
    if((x>=0) and (x<=1)):
        ...
        return 0
def tri_pdf(x):
    return np.convolve(uni_pdf(x),uni_pdf(x),mode='valid')
...
#plt.savefig('../figs/nat_pdf.pdf')
#plt.savefig('../figs/nat_pdf.eps')
